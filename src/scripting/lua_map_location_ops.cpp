...
static bool luaW_tocubeloc(lua_State* L, int idx, hex_location& out) {
	if(!lua_istable(L, idx)) {
		return false;
	}
	if(!luaW_tableget(L, -1, "q")) {
		return false;
	}
	out.q = luaL_checkinteger(L, -1);
	if(!luaW_tableget(L, -1, "r")) {
		return false;
	}
	out.r = luaL_checkinteger(L, -1);
	if(luaW_tableget(L, -1, "s")) {
...
static hex_location luaW_checkcubeloc(lua_State* L, int idx) {
	hex_location loc;
...
static void luaW_pushcubeloc(lua_State* L, hex_location loc) {
...
{
	hex_location h = luaW_checkcubeloc(L, 1);
